...
- The current engine input for excess CCC valuation is scalar (`cccMarketValuePct`). Ares XV's PPM says excess CCC should use the actual Market Value of the included excess obligations. The scalar `70.0` in `ppm.json` preserves the existing Euro XV fixture convention and has no current-period impact because the current compliance data is under the 7.5% Caa/CCC threshold with zero Caa/CCC adjustment. Forward CCC stress remains an approximation unless the engine is extended to carry the actual excess-bucket market-value calculation.
