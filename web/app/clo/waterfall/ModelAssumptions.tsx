...
const ASSUMPTIONS_REGISTER: { domain: string; items: Assumption[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Cap carryforward seed may be unknown", detail: "Senior Expenses Cap carryforward is historical unused cap headroom from prior payment dates. When historical trustee data is unavailable, the model uses a zero seed unless a user supplies the known amount. This affects senior-expense cap headroom only; it does not add cash.", impact: "low" },
      { label: "Reserve deposits are user elections", detail: "The PPM allows discretionary deposits into the expense reserve account at step (D) and supplemental reserve account at step (BB). The model supports fixed euro elections from available interest proceeds and defaults both to zero because historical manager elections require deal-specific data.", impact: "low" },
